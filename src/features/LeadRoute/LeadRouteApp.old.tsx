...
import { Box, Button, Dialog, DialogActions, DialogContent, DialogTitle, IconButton, InputAdornment, MenuItem, TextField, Typography } from '@mui/material';
    ...
    <Dialog open={Boolean(dialog)} onClose={close} maxWidth={dialog?.startsWith('csv') ? 'md' : 'sm'} fullWidth>
      ...
      <DialogContent>
        {(dialog === 'templateCreate' || dialog === 'templateEdit') && (
          <Box>
            <TextField label="Title" size="small" defaultValue={dialog === 'templateEdit' ? '1st' : ''} placeholder="e.g. Problem-Centric" autoFocus sx={{ mb: 2, width: '100%' }} />
            <TextField label="Value Prop Focus" size="small" defaultValue={dialog === 'templateEdit' ? '1st' : ''} placeholder="One-line angle" sx={{ mb: 2, width: '100%' }} />
            <TextField label="Subject" size="small" defaultValue={dialog === 'templateEdit' ? '1st' : ''} sx={{ mb: 2, width: '100%' }} />
            <TextField label="Body" multiline minRows={9} defaultValue={dialog === 'templateEdit' ? '1st' : ''} sx={{ width: '100%' }} />
          </Box>
        )}
        {dialog === 'salesImport' && (
          <Box>
            <TextField fullWidth label="Sales Navigator URL" size="small" defaultValue="https://www.linkedin.com/sales/lists/people/123456789  ·  /sales/search/people?keywords=..." autoFocus />
        ...
        {dialog === 'csvMap' && (
          <Box>
            {['Email *', 'First name', 'Last name', 'Company', 'Job title', 'LinkedIn URL'].map((field) => (
              <Box key={field} display="grid" gridTemplateColumns="150px 1fr 180px" gap={2} alignItems="center" py={1} borderBottom="1px solid var(--color-border)">
                <Typography>{field}</Typography>
                <TextField select size="small" defaultValue={field === 'LinkedIn URL' ? 'not' : field.toUpperCase().replace(' *', '').replace(' ', '')}>
        ...
        {dialog === 'prospectImport' && (
          <Box>
            ...
            <TextField fullWidth multiline minRows={5} defaultValue={`https://linkedin.com/in/jane-doe\nJohn Smith, john@acme.com, Acme Inc`} sx={{ mt: 2 }} />
          </Box>
        )}
      </DialogContent>
      <DialogActions>
