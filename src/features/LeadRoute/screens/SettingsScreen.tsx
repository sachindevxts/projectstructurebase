...
import { PageHeader, Tag, cx, integrations } from './LeadRouteScreenShared';
...
export function SettingsScreen() {
  ...
  const visible =
    filter === 'All'
      ? integrations
      : integrations.filter(
          (i) => i[2] === filter.toUpperCase() || (filter === 'AI Providers' && false),
        );
  ...
  return (
    <main className={styles.content}>
      ...
      <Box className={styles.buttonRow} my={2}>
        {['All', 'Connected', 'Lead Sources', 'Enrichment', 'CRM', 'AI Providers'].map((chip) => (
          <Button
            key={chip}
      ...
      <Box className={cx(styles.grid, styles.grid3)} mt={2}>
        {visible.map(([name, desc, cat, Icon, bg]) => (
          <Box className={cx(styles.panel, styles.integrationCard)} key={name as string}>
            <Box className={styles.popular}>
              <Tag>Popular</Tag>
            </Box>
            <Box className={styles.iconTile} sx={{ background: bg as string }}>
              <Icon size={17} />
            </Box>
